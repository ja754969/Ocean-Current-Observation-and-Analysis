...
clear;clc;clf
%% 
filename = '20180813_adt.txt';

fid = fopen(filename, 'r');

adt_20180813 = textscan(fid, '%s %f %f %f %f','headerLines', 10,'Delimiter',',');

fclose(fid);

adt_20180813 = cell2mat(adt_20180813(3:5));
% adt_20180813的行資料 : xlon ylat adt(absolute dynamic topography)
%% Set variables
xlon = adt_20180813(:,1);
ylat = adt_20180813(:,2);
adt = adt_20180813(:,3); % adt(absolute dynamic topography)
...
hold off;
title('20180813_adt','Interpreter','none')
print('W11_class01_20180813_adt','-dpng')
...
kuroshio_avg_spd = nanmean(spd(spd>=0.6)); % 計算黑潮流軸平均流速
...
xlabel({['Flow field mean speed : ' avg_spd_str(1:5) ' (m/s)'];...
    ...
    ,'color','b','fontsize',10);
title('20180813_adt_current_speed','Interpreter','none')
print('W11_class02_20180813_adt','-dpng')